...
# dev_cluster.sh
# Start one or more FerrisSearch nodes for local development.
#
# Usage:
#   ./dev_cluster.sh <node_id>     # Start a single node
#   ./dev_cluster.sh --nodes 3     # Start a 3-node cluster (background, Ctrl+C stops all)
...
start_single_node() {
    ...
    local HTTP_PORT=$((9200 + OFFSET))
    local TRANSPORT_PORT=$((9300 + OFFSET))
    ...
    echo "Starting FerrisSearch Node $NODE_ID (Raft ID $NODE_ID)..."
    echo "HTTP Port: $HTTP_PORT | Transport Port: $TRANSPORT_PORT"

    cargo run --bin ferrissearch
}
...
start_cluster() {
    ...
    echo
    echo "$NUM_NODES-node cluster running. Press Ctrl+C to stop all nodes."
    wait
}

if [[ "$1" == "--nodes" ]]; then
    if [[ -z "$2" ]] || ! [[ "$2" =~ ^[0-9]+$ ]]; then
        echo "Usage: ./dev_cluster.sh --nodes <count>"
        exit 1
    fi
    start_cluster "$2"
elif [[ -n "$1" ]] && [[ "$1" =~ ^[0-9]+$ ]]; then
    ...
    echo "Usage: ./dev_cluster.sh <node_id>       # Start a single node"
    echo "       ./dev_cluster.sh --nodes <count>  # Start a multi-node cluster"
    echo
    echo "Examples:"
    echo "  ./dev_cluster.sh 1          # Start node 1 on port 9200"
    echo "  ./dev_cluster.sh --nodes 3  # Start 3-node cluster"
    exit 1
fi
