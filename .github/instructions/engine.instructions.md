...
- **Collision-free composite keys**: Single-column GROUP BY uses `OrdHashMap<u64>` (identity hasher). Width-2 GROUP BY uses a packed `(u64, u64)` key in `HashMap<u128, ...>`. Width 3+ falls back to `HashMap<Vec<u64>>` (exact ordinal match). No hash collisions are introduced by the key encoding.
