...
applyTo: "src/hybrid/**,src/api/search/**,src/engine/tantivy.rs"
---
...
- `execute_sql_query()` in `src/api/search/mod.rs` is the shared internal function used by both `search_sql` and `explain_sql(analyze=true)`. Timing instrumentation is in this one place.
