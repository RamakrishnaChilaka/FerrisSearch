...
- Streamed SQL responses use `application/x-ndjson` with a first `meta` frame and subsequent `rows` frames. Execution metadata (`execution_mode`, `streaming_used`, `planner`, `_shards`, `matched_hits`, `columns`, `truncated`) stays in the `meta` frame, and buffered/re-framed SQL paths may also attach a `timings` object there so ferris-cli can render grouped-partials timing breakdowns from the streamed response.
